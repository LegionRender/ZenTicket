...
import { isBypassForbidden, authenticateFirebaseToken } from "./middleware/auth.middleware";
import { requireAdmin } from "./middleware/admin.middleware";
import adminDiagnosticsRoutes from "./routes/adminDiagnostics.routes";
...
import { verifyStripeSignature } from "./utils/crypto.utils";
...
app.post("/api/tickets/analyze", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
  try {
...
app.post("/api/tickets/train-jit", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    let portalUrl = "";
    let verifiedPortalSelectors: string[] = [];
    const portalCandidates: string[] = [];
    const ticketPortalCandidates = new Set<string>();
    const addPortalCandidate = (candidate: unknown, source: "ticket" | "search" = "search") => {
    ...
    const brandDomainToken = nombreEmisor.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      .split(/[^a-z0-9]+/).filter((token: string) => token.length >= 4 && !["tienda", "tiendas", "cadena", "comercial", "grupo"].includes(token))
    ...
    try {
      ...
      const groundingChunks = (searchResponse as any).candidates?.[0]?.groundingMetadata?.groundingChunks || [];
      ...
      const officialChunks = (officialResponse as any).candidates?.[0]?.groundingMetadata?.groundingChunks || [];
    ...
    const merchantTokens = nombreEmisor.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
      .split(/[^a-z0-9]+/).filter((token: string) => token.length >= 4);
    ...
    for (const candidate of portalCandidates) {
      try {
        ...
        if (response.ok || [401, 403].includes(response.status)) {
          ...
          const merchantDomainMatch = merchantTokens.some((token: string) => hostname.includes(token));
          ...
          for (const hrefMatch of html.matchAll(/href=["']([^"'#]+)["']/gi)) {
            try {
              ...
              if (merchantTokens.some((token: string) => linkedUrl.hostname.toLowerCase().includes(token))) {
    ...
    const learningMemory = learningMemorySnap.docs.map((doc: any) => doc.data());
    ...
    try {
      ...
      Patrones operativos previos del mismo dominio: ${JSON.stringify(learningMemory).slice(0, 4000)}
      
      Genera una propuesta provisional que después será reemplazada por el análisis del DOM real. No inventes campos ni URLs:
      ...
      Los campos y selectores deben confirmarse posteriormente contra el DOM; no agregues campos por analogía con otros comercios.`;
      ...
      try {
        ...
        if (visiblePasswordFields > 0 || visibleLoginControls > 0 ||
          ...
          const authError: any = new Error("El portal oficial requiere una cuenta o sesión del comercio para facturar.");
        ...
        portalMetadata = await page.evaluate(() => ({
          framework: document.querySelector("[ng-version]") ? "Angular" :
            ...
            (window as any).jQuery ? "jQuery" : "unknown",
          iframes: Array.from(document.querySelectorAll("iframe")).filter(el => el.getBoundingClientRect().width > 0)
            .map(el => ({ src: (el as HTMLIFrameElement).src, sandbox: el.getAttribute("sandbox") || "" })),
          modals: Array.from(document.querySelectorAll("[role='dialog'], .modal.show, dialog[open]"))
            .map(el => ({ text: (el.textContent || "").trim().slice(0, 300) })),
          datepickers: Array.from(document.querySelectorAll("input[type='date'], .flatpickr-input, [data-provide='datepicker']"))
            .map(el => ({ id: el.id || "", name: el.getAttribute("name") || "" })),
          submitButtons: Array.from(document.querySelectorAll("button[type='submit'], input[type='submit']"))
            .map(el => ({ text: (el.textContent || el.getAttribute("value") || "").trim() }))
        }));
        ...
        const discoveredInputs = await page.evaluate(() => {
          return Array.from(document.querySelectorAll(
            ...
            type: el.getAttribute("type") || "",
            placeholder: el.getAttribute("placeholder") || "",
            selector: el.id ? `#${CSS.escape(el.id)}` : (el as HTMLInputElement).name ? `[name="${CSS.escape((el as HTMLInputElement).name)}"]` : "",
            labelText: (() => {
              if (el.id) {
                const lbl = document.querySelector(`label[for="${el.id}"]`);
        ...
        verifiedPortalSelectors = [...new Set([
          ...discoveredInputs.map((input: any) => input.selector),
          ...discoveredActions.map((action: any) => action.selector)
        ...
        if (discoveredInputs.length > 0) {
          ...
          const refineResponse = await ai2.models.generateContent({
            ...
            contents: [
              ...
              Propuesta provisional (no confíes en sus campos ni selectores):
        ...
        const verificationError: any = new Error(`Playwright no pudo verificar el portal: ${playwrightErr.message}`);
        verificationError.code = playwrightErr.code;
        throw verificationError;
      }
    } catch (discoveryErr: any) {
      const wrappedError: any = new Error(`No fue posible construir un conector verificable: ${discoveryErr.message}`);
      wrappedError.code = discoveryErr.code;
      throw wrappedError;
    }


    discoverResult.requiredPortalFields = (discoverResult.requiredPortalFields || []).map((field: any) => {
      const key = String(field.key || field.canonicalKey || "").replace(/^portalFields\./, "").trim();
      return { ...field, key, canonicalKey: key };
    });
    const fiscalKeyAliases: Record<string, string> = {};
    discoverResult.fiscalFields = (discoverResult.fiscalFields || []).map((field: any) => {
    ...
    const normalizeAndValidateSteps = (source: any): any[] => {
      ...
      const normalized = rawSteps.map((raw: any) => {
      ...
      if (!normalized.some((step: any) => step.type === "goto")) normalized.unshift({ type: "goto", url: portalUrl });
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Gemini generó un mapa de navegación inválido: ${repairError.message}`);
      }
    }
    ...
    const portalMapData = {
      ...
      errorPatterns: discoverResult.errorPatterns || [
        ...
        { textPattern: "total incorrecto", category: "INVALID_TOTAL", recoveryHint: "Probar formatos numéricos alternativos" },
    ...
    const fiscalProfileSnapshot: any = {
      userId: decodedToken.uid,
    ...
    if (missingFiscalFields.length === 0) {
      ...
      const existingActiveJob = existingJobs.docs.find((doc: any) =>
      ...
      await adminDb.collection("tickets").doc(ticketId).update({
        ...
        jobId: invoiceJobId,
        reviewReasonCode: FieldValue.delete(),
        reviewError: FieldValue.delete(),
...
app.post("/api/admin/analyze-html", authenticateFirebaseToken, requireAdmin, async (req: Request, res: Response): Promise<void> => {
...
app.post("/api/cfdi/verify-sat", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
            ...
            if (verification.status === "valid") {
              if (finalInvoiceId) {
                ...
                if (invSnap.exists) {
                  ...
                  // Create real invoice if missing
                  const newInvoice = {
                    ...invUpdates,
                    userId: uid,
                    ticketId: ticketId,
...
app.post("/api/tickets/:ticketId/retry-invoice-recovery", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Verificar si ya existe una invoice real validada
    let targetInvoice: any = null;
    let invoiceId: string = ticketData.invoiceId || "";
    if (invoiceId) {
    ...
    // Reiniciar recoveryAttemptCount y preparar ciclo controlado
    const recoveryAttemptCount = 0;
    const nextRecoveryAt = new Date().toISOString(); // Programar de inmediato!
    ...
    await ticketRef.update({
      ...
      manualRecoveryRequestedAt: new Date().toISOString(),
      manualRecoveryRequestedBy: uid,
      errorCode: null,
      reviewReasonCode: null,
      errorMsg: "Recuperación de factura solicitada manualmente por el usuario.",
      updatedAt: new Date().toISOString()
    });

    // Encontrar y resetear el job asociado, o crear uno nuevo si no existe!
    ...
    res.json({
      success: true,
      message: "Recuperación manual de factura encolada correctamente.",
      ticketId,
      jobId
    });
  } catch (error: any) {
    console.error("[retry-invoice-recovery] Error:", error);
    res.status(500).json({ error: "Error interno del servidor al reintentar la recuperación." });
  }
});

// API endpoint: Run high-fidelity automation simulation and generate official CFDI XML & PDF html representation
app.post("/api/automation/run", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
...
app.post("/api/email/send", authenticateFirebaseToken, async (req: Request, res: Response): Promise<void> => {
...
import { getSafeBaseUrl } from "./utils/url.utils";
...
async function getUserIdByStripeCustomerId(stripeCustomerId: string): Promise<string | null> {
  const snap = await adminDb.collection("billingProfiles")
  ...
  const snapFiscal = await adminDb.collection("fiscalProfiles")
    .where("stripeCustomerId", "==", stripeCustomerId)
    .limit(1)
    .get();
  if (!snapFiscal.empty) {
...
async function syncSubscriptionInDb(subscription: any, stripeCustomerId: string) {
  ...
  console.log(`[Stripe Webhook] Sincronizada suscripción ${subscription.id} para usuario ${userId} (status: ${status})`);
}

async function cancelSubscriptionInDb(subscription: any, stripeCustomerId: string) {
  const userId = subscription.metadata?.userId || await getUserIdByStripeCustomerId(stripeCustomerId);
  if (!userId) {
  ...
  console.log(`[Stripe Webhook] Cancelada suscripción ${subscription.id} para usuario ${userId}`);
}
...
const handleSetDefaultPaymentMethod = async (req: any, res: any) => {
  const userId = req.user.uid;
  const email = req.user.email;
  const emailVerified = req.user.email_verified;
  ...
  try {
    ...
    // VALIDATION: check if the PaymentMethod belongs to this customer on Stripe
    try {
      ...
      if (pmDetails.customer && pmDetails.customer !== stripeCustomerId) {
        res.status(403).json({ error: "No tienes permisos para usar este método de pago." });
        return;
      }
    } catch (pmErr: any) {
