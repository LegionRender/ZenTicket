...
app.post("/api/connectors/learn", authenticateFirebaseToken, async (_req: Request, res: Response): Promise<void> => {
  res.status(410).json({
    code: "JIT_GOVERNANCE_FROZEN",
    error: "El aprendizaje heurístico de conectores está retirado."
  });
});
app.post("/api/admin/discover-portal", authenticateFirebaseToken, requireAdmin, async (_req: Request, res: Response): Promise<void> => {
  res.status(410).json({
    code: "JIT_GOVERNANCE_FROZEN",
    error: "El discovery administrativo de portales está retirado."
  });
});
app.post("/api/tickets/train-jit", authenticateFirebaseToken, async (_req: Request, res: Response): Promise<void> => {
  res.status(410).json({
    code: "JIT_GOVERNANCE_FROZEN",
...
import { parseSatQrUrl } from "../firebase/functions/fiscalUtils.js";

app.post("/api/cfdi/verify-sat", authenticateFirebaseToken, async (_req: Request, res: Response): Promise<void> => {
  res.status(410).json({
