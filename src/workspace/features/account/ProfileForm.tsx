...
import { parseFiscalConstancia } from "@/services/api/fiscalService";
import type { ProfileFormProps } from "@/workspace/features/account/account.types";
import {
  buildDefaultPaymentCards,
  buildLegionPrimaryCard,
  ...
  isLegionUserEmail,
  isValidLuhn,
  normalizeLegionCards,
} from "@/workspace/features/account/accountUtils";
...
export default function ProfileForm({ 
    ...
    try {
      await onSave(
        buildPlanSavePayload({
          ...
          razonSocial: razonSocial?.trim().toUpperCase() || initialProfile?.razonSocial || DEFAULT_FISCAL_PROFILE.razonSocial,
  ...
  const [rfc, setRfc] = useState(initialProfile?.rfc || DEFAULT_FISCAL_PROFILE.rfc);
  const [razonSocial, setRazonSocial] = useState(initialProfile?.razonSocial || DEFAULT_FISCAL_PROFILE.razonSocial);
  const [regimenFiscal, setRegimenFiscal] = useState(initialProfile?.regimenFiscal || DEFAULT_FISCAL_PROFILE.regimenFiscal);
  const [codigoPostal, setCodigoPostal] = useState(initialProfile?.codigoPostal || DEFAULT_FISCAL_PROFILE.codigoPostal);
  const [usoCFDI, setUsoCFDI] = useState(initialProfile?.usoCFDI || DEFAULT_FISCAL_PROFILE.usoCFDI);
  const [hasSavedFiscalData, setHasSavedFiscalData] = useState(() => {
    return !isMockFiscalProfile(initialProfile);
  });
  ...
  const handleConstanciaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await parseFiscalConstancia(base64, mimeType, personalGeminiKey);
  ...
  const [cards, setCards] = useState<PaymentCard[]>(() => buildDefaultPaymentCards(initialProfile));
  ...
  React.useEffect(() => {
    ...
    const isLegion = isLegionUserEmail(userEmail);
    ...
    if (isLegion) {
      ...
      const { hasRealCard, hasMockDaniels, normalizedCards } = normalizeLegionCards(cards);

      if (!hasRealCard || hasMockDaniels) {
        const listToSave = normalizedCards;
  ...
  const [nombreCompleto, setNombreCompleto] = useState(DEFAULT_FISCAL_PROFILE.fallbackDisplayName);
  const [correoElectronico, setCorreoElectronico] = useState(DEFAULT_FISCAL_PROFILE.fallbackDisplayEmail);
  const [telefono, setTelefono] = useState("+52 55 1234 5678");
  const [correoRecepcion, setCorreoRecepcion] = useState(
    initialProfile?.correoRecepcion || currentUserEmail || auth.currentUser?.email || DEFAULT_FISCAL_PROFILE.fallbackReceptionEmail
  );
  ...
  React.useEffect(() => {
    if (initialProfile) {
      ...
      setHasSavedFiscalData(
        !!(
          initialProfile.rfc &&
          initialProfile.razonSocial &&
          initialProfile.codigoPostal &&
          !isMockFiscalProfile(initialProfile)
        )
      );
    }
  }, [initialProfile]);
    ...
    try {
      await onSave(
        buildProfileSavePayload({
          ...
          metodoRecepcion,
          extra: { navigationDisabled: false }, // Deactivate navigation restriction automatically so user can navigate!
        })
      );
  ...
  if (isEditingFiscal) {
    return (
      <div id="fiscal-form-pane" className="max-w-3xl mx-auto bg-[#F8F9FE] border border-slate-200/50 shadow-sm rounded-3xl p-5 sm:p-8 animate-fade-in_50 font-sans text-left mt-2 relative select-none">
        ...
        <div className="bg-gradient-to-br from-blue-50/50 to-white border border-[#0B53F4]/20 rounded-3xl p-5 mb-5 shadow-2xs text-left animate-fade-in">
        ...
        <div className="mb-8">
          <button
            ...
            className="w-full bg-[#0B53F4] hover:bg-[#0747D1] disabled:bg-slate-300 disabled:text-slate-500 disabled:opacity-90 disabled:shadow-none text-white text-xs font-black py-4 rounded-2.5xl transition shadow-[var(--shadow-surface)] cursor-pointer text-center flex items-center justify-center gap-2 active:scale-98 disabled:cursor-not-allowed"
          >
  ...
  return (
    <div id="account-tab-dashboard" className="w-full space-y-5 font-body text-left animate-fade-in_50 pb-20 select-none">
      ...
      <div className="bg-white border border-slate-200/60 rounded-[1.15rem] p-4 shadow-[var(--shadow-surface)] flex items-center justify-between font-body">
      ...
      <div className="space-y-2.5">
        ...
        <div className="bg-white border border-slate-200/60 rounded-[1.15rem] p-4 shadow-[var(--shadow-surface)] space-y-4">
      ...
      <div className="lg:col-span-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="group flex items-center justify-between gap-1.5 py-1.5 px-3 border border-slate-200 bg-slate-50/70 hover:bg-slate-100 active:bg-slate-200 text-[#0B53F4] font-extrabold text-[9.5px] uppercase tracking-wider rounded-xl transition-all duration-150 cursor-pointer shadow-3xs select-none"
          >
            <span>{addingCard ? "Cancelar" : "+ Agregar Método de Pago"}</span>
            <ChevronRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#0B53F4] transition-transform duration-150 transform group-hover:translate-x-0.5" />
          </button>
        </div>
        ...
        {addingCard && (
          <div className="bg-slate-50 border border-slate-200/80 rounded-3xl p-5 mb-4 animate-fade-in text-left space-y-4">
            ...
            {(() => {
              ...
              return (
                <div className={`bg-gradient-to-br ${bankInfo.bgColor} text-white rounded-2xl p-5 relative overflow-hidden shadow-[var(--shadow-surface)] font-mono select-none h-38 flex flex-col justify-between transition-all duration-300 border border-white/10`}>
        ...
        <div className="bg-white border border-slate-200/60 rounded-[1.15rem] overflow-hidden divide-y divide-slate-100 shadow-[var(--shadow-surface)]">
          {cards.length === 0 ? (
      ...
      <div className="space-y-2.5">
        ...
        <div className="bg-white border border-slate-200/60 rounded-[1.15rem] overflow-hidden divide-y divide-slate-100 shadow-[var(--shadow-surface)]">
          ...
          <div 
            ...
            className="w-full p-4 flex items-center justify-between gap-3 hover:bg-slate-50 transition text-left cursor-pointer active:scale-[0.99]"
          >
            ...
            <span className="flex-1 text-sm font-bold text-slate-800 tracking-tight">FAQ</span>
            <ChevronRight className="w-4 h-4 text-slate-350" />
          </div>
          ...
          >
            ...
            <span className="flex-1 text-sm font-bold text-slate-800 tracking-tight">Tutoriales</span>
            <ChevronRight className="w-4 h-4 text-slate-350" />
          </div>
          ...
          >
            ...
            <span className="flex-1 text-sm font-bold text-slate-800 tracking-tight">Soporte</span>
            <ChevronRight className="w-4 h-4 text-slate-350" />
          </div>
        </div>
      </div>
          ...
          <div className="bg-rose-50 border border-rose-200 rounded-2.5xl p-5 space-y-3.5 text-left animate-fade-in">
            ...
            <div className="flex gap-2.5 pt-1">
              ...
              <button
                ...
                className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl uppercase tracking-wider transition cursor-pointer"
              >
                Cancelar
              </button>
            </div>
      ...
      {selectedDetailsCard && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl overflow-hidden max-w-md w-full shadow-[var(--shadow-elevated)] border border-slate-100 flex flex-col text-slate-800 animate-scale-up">
            ...
            <div className="p-6 text-left space-y-6">
              {(() => {
                ...
                return (
                  <div className={`bg-gradient-to-br ${bgInfo} text-white rounded-2xl p-5 relative overflow-hidden shadow-[var(--shadow-surface)] font-mono select-none h-38 flex flex-col justify-between border border-white/10`}>
                    <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-tr from-white/10 to-transparent rounded-full blur-xl pointer-events-none" />
                    <div className="flex justify-between items-start z-10">
                      <div className="text-left">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-50 rounded-2xl p-3 flex flex-col gap-2 border border-slate-200/50 text-center">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="bg-white hover:bg-slate-50 text-[10px] text-slate-700 font-bold py-1.5 px-2 rounded-xl border border-slate-200 transition cursor-pointer text-center"
                      >
                        OTP: 481920
                      </button>
                      <button
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      ...
                      className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-650 text-xs font-black py-3 rounded-xl transition cursor-pointer text-center"
                    >
                      Cancelar
                    </button>
                    <button
